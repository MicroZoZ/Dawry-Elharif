...
router.post("/:quizId", async(req,res)=>{
    ...
    if (!(Quiz.state === "in-progress")){
        if(attempt&&!(attempt.completed)){return finishQuiz(res,attempt,student,new Date(attempt.startedAt.getTime()+(20*60*1000)),quizId,"your exam time has been expired")}
    ...
    if(attempt.currentQuestionIndex >= questions.length) {
            ...
            return await finishQuiz(res,attempt,student,new Date((attempt.startedAt).getTime()+(20*60*1000)),quizId,message);
        }
    const startedAt = new Date(attempt.startedAt)
    const totalTimeByMinutes = Quiz.totalTimeByMinutes
    const expiresAt = new Date(startedAt.getTime()+(totalTimeByMinutes*60*1000))
